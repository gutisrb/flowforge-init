...
export default function Furnisher() {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const response = await fetch(
          `${MAKE_STATUS_URL}?jobId=${encodeURIComponent(jid)}`,
          { headers: { 'x-make-apikey': MAKE_API_KEY }, cache: 'no-store' }
        );
  ...
  const handleGenerate = async () => {
    ...
    if (!style.trim()) {
      toast({ title: 'Error', description: 'Please enter a style.', variant: 'destructive' });
      return;
    }
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Short guidance (colors, key furniture, layout, mood)…"
                  rows={3}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={type} onValueChange={setType}>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Label htmlFor="old">Old / for renovation</Label>
                  </div>
                </RadioGroup>
              </div>
